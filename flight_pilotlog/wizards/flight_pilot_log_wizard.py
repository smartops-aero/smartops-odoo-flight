...
class FlightPilotLog(models.TransientModel):
    ...
    flight_id = fields.Many2one(
        "flight.flight", string="Flight", required=True, readonly=True
    )
    ...
    def _compute_partner_id(self):
        ...
        for record in self:
            ...
            if record.flight_id:
                ...
                pic_role = self.env["flight.crew.role"].search(
                    [("name", "ilike", "Pilot-in-command")], limit=1
                )
            ...
            if not partner and record.flight_id:
                ...
                pilot_time = self.env["flight.pilot.time"].search(
                    [("flight_id", "=", record.flight_id.id)], limit=1
                )
                if pilot_time:
