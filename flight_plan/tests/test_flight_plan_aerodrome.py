...
class TestFlightPlanAerodrome(TransactionCase):
    def setUp(self):
        ...
        self.flight_plan = self.env["flight.plan"].create(
            {
                "flight_id": self.env["flight.flight"]
                .create(
                    {
                        "date": "2025-01-01",
                        "aircraft_id": self.env["flight.aircraft"]
                        .create(
                            {
                                "registration": "N123TEST",
                                "model_id": self.env["flight.aircraft.model"]
                                .create({"name": "Test Aircraft Model"})
                                .id,
                            }
                        )
                        .id,
                        ...
                        .create(
                            {
                                ...
                                "model_id": self.env["flight.aircraft.model"]
                                .create({"name": "Test Aircraft Model 2"})
                                .id,
                            }
                        )
                        .id,
