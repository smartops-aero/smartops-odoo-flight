...
def create_spec(cr, aircraft_id, code, value, user_id, is_bool=False, category_id=None):
    ...
    try:
        ...
        _logger.info(
            f"Created spec {new_spec_id} for aircraft {aircraft_id}, code {code}"
        )

    except Exception as e:
        _logger.error(
            f"Error creating spec for aircraft {aircraft_id}, code {code}: {str(e)}"
        )
...
def migrate(cr, version):
    ...
    cr.execute(
        "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'website_fleet_migration')"
    )
    ...
    if not cr.fetchone()[0]:
        _logger.error(
            "Migration table does not exist! Pre-migration might have failed."
        )
        return
    ...
    _logger.info(
        f"Found {len(old_amenity_names)} unique amenities to migrate: {old_amenity_names}"
    )
    ...
    if unmapped_amenities:
        _logger.warning(
            f"The following amenities could not be mapped: {', '.join(unmapped_amenities)}"
        )
