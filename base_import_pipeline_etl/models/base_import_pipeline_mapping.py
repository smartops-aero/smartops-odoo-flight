...
class ImportPipelineMapping(models.Model):
    ...
    pipeline_id = fields.Many2one(
        "base.import.pipeline", string="Pipeline", required=True, ondelete="cascade"
    )
    sequence = fields.Integer(default=10)
    source_field = fields.Char(
        string="Source Field", required=False, help="Field name in the source data"
    )
    ...
    relation_model = fields.Char(
        related="relation_model_id.model", string="Related Model Name", store=True
    )
