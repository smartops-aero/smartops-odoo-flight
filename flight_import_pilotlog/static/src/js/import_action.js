          ...
          .then(() => {
            ...
            if (transformerId) {
              this._showLoadingIndicator(
                _t("Preparing transformation preview")
              );
        ...
        .then((result) => {
          ...
          if (result && result.status === "success") {
            ...
            this.do_warn(
              _t("Transformation Error"),
              (result && result.message) ||
                _t("Failed to update transformation")
            );
          }
        })
        .guardedCatch((error) => {
    ...
    onpreview_success: function (event, from, to, result) {
      ...
      if (FLIGHT_MODELS.includes(this.res_model)) {
        ...
        if (result.transformer_id) {
          this.$("select.flight_import_transformer").val(
            result.transformer_id.id
          );
        ...
        if (result.is_transformed) {
          const transformerName = this.$(
            "select.flight_import_transformer option:selected"
          ).text();
          const $notice = $("<div>", {
          ...
          this.$(".oe_import_box").append(
            $notice.addClass("flight_import_transform_notice")
          );
        } else {
          this.$(".flight_import_transform_notice").remove();
        }
