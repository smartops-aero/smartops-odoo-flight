...
export class RelativeDateTimePicker extends DateTimePicker {
    ...
    formatValue(value, options) {
        ...
        if (!error && formattedValue !== "" && this.baseDate && options.format.includes('%R')) {
            const deltaDays = value.startOf('day').diff(this.baseDate.startOf('day'), "days").days;
            if (deltaDays !== 0) {
    ...
    parseValue(value, options) {
        const [timeString, deltaDays, timezone] = value.split(/\s*(?:([+-]\d+)|([A-Z]{3,4}))\s*$/);
        ...
        if (!isNaN(hours) && !isNaN(minutes)) {
            if (timezone) {
                ...
                parsedDate = parsedDate.set({hours, minutes, seconds: 0, milliseconds: 0});
            }
