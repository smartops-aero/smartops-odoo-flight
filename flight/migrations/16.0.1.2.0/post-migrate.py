...
def migrate(cr, version):
    ...
    for role in roles:
        if role.name in unique_roles:
            ...
            _logger.info('Found duplicate role "%s" (ID: %d), original has ID: %d', 
                        role.name, role.id, unique_roles[role.name])
        else:
    ...
    cr.execute("SELECT 1 FROM information_schema.columns WHERE table_name = 'flight' AND column_name = 'lock_state'")
    if cr.fetchone():
        has_lock_field = True
    ...
    cr.execute("SELECT 1 FROM information_schema.columns WHERE table_name = 'flight_crew_role' AND column_name = 'active'")
    if cr.fetchone():
        has_active_field = True
        _logger.info('flight.crew.role model has active field, will use it for duplicates that cannot be removed')
    ...
    for duplicate in duplicates:
        ...
        if crew_count == 0:
            ...
            _logger.info('Role "%s" (ID: %d) is not used in any flight crew, safe to remove', 
                        duplicate['name'], dup_id)
            continue
        ...
        if has_lock_field:
            ...
            if locked_count == 0:
                ...
                cr.execute("UPDATE flight_crew SET role_id = %s WHERE role_id = %s", 
                          (original_id, dup_id))
                
                _logger.info('Updated %d flight crew records from role ID %d to %d', 
                            crew_count, dup_id, original_id)
            else:
                # Used in locked flights, cannot remove
                cannot_remove.append(dup_id)
                _logger.info('Role "%s" (ID: %d) is used in %d locked flights, cannot remove', 
                            duplicate['name'], dup_id, locked_count)
        else:
            # Cannot determine if flights are locked, assume cannot remove
            cannot_remove.append(dup_id)
            _logger.info('Role "%s" (ID: %d) is used in flights and lock status cannot be determined, cannot remove', 
                        duplicate['name'], dup_id)
    ...
    if cannot_remove and has_active_field:
        ...
        cr.execute(f"UPDATE flight_crew_role SET active = false WHERE id IN ({ids_str})")
        _logger.info('Set %d duplicate roles as inactive (used in locked flights)', len(cannot_remove))
    
    # Log completion
    _logger.info('Migration completed. Removed %d duplicates, made %d inactive.', 
                len(safe_to_remove), len(cannot_remove) if has_active_field else 0)
