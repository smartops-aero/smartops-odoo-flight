...
class TestFlight(FlightCommon):
    ...
    def test_06_flight_multi_record_operations(self):
        ...
        for i in range(3):
            flights |= self.create_test_flight(date=date.today() + timedelta(days=i))
    ...
    def test_07_flight_search_filters(self):
        ...
        # Test locked filter
        locked_flights = self.env["flight.flight"].search([("locked", "=", True)])
        ...
        grouped = self.env["flight.flight"].read_group(
            [("aircraft_id", "!=", False)], ["aircraft_id"], ["aircraft_id"]
        )
