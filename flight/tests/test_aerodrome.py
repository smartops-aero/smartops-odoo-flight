...
class TestAerodrome(FlightCommon):
    ...
    def test_02_aerodrome_display_name(self):
        ...
        # With IATA code
        self.assertEqual(
            self.aerodrome_jfk.display_name, "KTES(TSE) - Test Airport East"
        )
        ...
        self.assertEqual(
            aerodrome_iata_only.display_name, "KIOA(IOA) - IATA Only Airport"
        )
    ...
    def test_04_aerodrome_search_by_location(self):
        ...
        # Search by city
        found = self.env["flight.aerodrome"].search([("city", "=", "New York")])
        ...
        # Search by ICAO
        found = self.env["flight.aerodrome"].search([("icao", "=", "KTES")])
    ...
    def test_08_aerodrome_as_departure_arrival(self):
        ...
        self.env["flight.flight"].search([("departure_id", "=", self.aerodrome_jfk.id)])

        self.env["flight.flight"].search([("arrival_id", "=", self.aerodrome_lax.id)])
