...
class TestSecurity(FlightCommon):
    ...
    def test_02_flight_read_access(self):
        ...
        for user in [self.user_manager, self.user_dispatcher, self.user_crew, self.user_basic]:
    ...
    def test_06_aircraft_access_rights(self):
        ...
        aircraft = self.env['flight.aircraft'].with_user(self.user_dispatcher).create(vals)
    ...
    def test_07_aerodrome_access_rights(self):
        ...
        aerodrome1 = self.env['flight.aerodrome'].with_user(self.user_manager).create(vals)
        self.assertTrue(aerodrome1.id)
        
        vals['icao'] = 'KTS2'
        aerodrome2 = self.env['flight.aerodrome'].with_user(self.user_dispatcher).create(vals)
