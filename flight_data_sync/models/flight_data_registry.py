...
class FlightDataRegistry(models.Model):
    ...
    provider_id = fields.Many2one('flight.data.provider', string='Data Provider', required=True)
    model = fields.Selection(selection='_selection_model', string='Model', required=True)
    ...
    _sql_constraints = [
        ('unique_registry_entry', 'unique(provider_id, model, external_id)',
    ...
    @api.model
    def get_or_create_local_id(self, provider_id, model, external_id, external_provider_id, values):
