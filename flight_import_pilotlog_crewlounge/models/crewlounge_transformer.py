...
class FlightImportPIlotlogTransformer(models.Model):
    ...
    def _get_available_implementations(self):
        ...
        implementations = super(
            FlightImportPIlotlogTransformer, self
    ...
    def flight_flight_crewlounge_transform_data(
        ...
        transformed_headers = [
            ...
            "arrival_id/icao",
            "remark_ids/id",
            "remark_ids/partner_id/id",
            "remark_ids/remark",
            "pilot_time_ids/id",
            "pilot_time_ids/partner_id/id",
            "pilot_time_ids/code_id/id",
            "pilot_time_ids/duration",
            "pilot_event_ids/id",
            "pilot_event_ids/partner_id/id",
            "pilot_event_ids/event_code_id/id",
            "pilot_event_ids/count",
        ]
        ...
        for idx, row in enumerate(data_rows):
            ...
            if (
                ...
                and all(
                    str(row[i]).upper() == str(headers[i]).upper()
            ...
            try:
                ...
                if date_idx is not None and date_idx < len(row) and row[date_idx]:
                    try:
                        ...
                        date_str = datetime.strptime(
                            str(row[date_idx]), "%d-%m-%Y"
                        ).strftime("%Y-%m-%d")
                    except (ValueError, TypeError) as e:
                        _logger.warning(
                            "Row %d: Failed to parse date '%s': %s. Skipping date.",
                            idx + 1,
                            row[date_idx] if date_idx < len(row) else "N/A",
                            e,
                        )
                ...
                if not date_str or not aircraft_reg or not departure or not arrival:
                    _logger.warning(
                        "Row %d: Missing essential flight data (Date, Reg, Dep, Arr). Skipping row.",
                        idx + 1,
                    )
                    skipped_rows += 1
                    continue
                        ...
                        try:
                            ...
                            if special_format == "HH:MM":
                                ...
                                if ":" in time_str:
                                    ...
                                    duration_hours = self._round_duration(
                ...
                for csv_col, odoo_code_xmlid in EVENT_CODE_MAPPING.items():
                    ...
                    if col_idx is not None and col_idx < len(row) and row[col_idx]:
                        try:
                            count = int(
                                float(row[col_idx])
                            )  # Use float first for potential decimals like '1.0'
                            if count > 0:
                                events_data.append(
                                    {
                ...
                max_lines = max(
                    1, num_remarks, num_times, num_events
                )  # Need at least 1 row for the flight
                ...
                for i in range(max_lines):
                    ...
                    time_duration_str = (
                        str(time_duration) if time_duration != "" else ""
                    )
                    ...
                    transformed_row = [
                        ...
                        str(remark_id),
                        str(remark_partner),
                        str(remark_text),  # Ensure strings
                        str(time_id),
                        str(time_partner),
                        str(time_code),
                        time_duration_str,  # Use string version
                        str(event_id),
                        str(event_partner),
                        str(event_code),
                        event_count_str,  # Use string version
                    ]
        ...
        if skipped_rows > 0:
            _logger.warning(
                "%d rows were skipped due to errors or missing essential data. Please check logs.",
                skipped_rows,
            )
    ...
    def flight_aircraft_crewlounge_transform_data(
        ...
        transformed_headers = ["id", "registration", "operator_id", "model_id"]
        ...
        for idx, row in enumerate(data_rows):
            ...
            try:
                ...
                if reg_idx is None or reg_idx >= len(row) or not row[reg_idx]:
                    _logger.warning(
                        "Row %d: Missing aircraft registration. Skipping row.", idx + 1
                    )
                    skipped_rows += 1
                    continue
                ...
                if aircraft_reg in processed_aircraft:
                    _logger.info(
                        "Skipping duplicate aircraft registration: %s", aircraft_reg
                    )
                    continue
                ...
                full_model_name = self._create_full_model_name(
                    make_name, model_name, variant
                )

                # --- Extract Operator ---
                operator_idx = header_map.get("OPERATOR")
                operator_name = (
                    str(row[operator_idx]).strip()
                    if operator_idx is not None
                    and operator_idx < len(row)
                    and row[operator_idx]
                    else ""
                )
    ...
    def flight_aircraft_model_crewlounge_transform_data(
        ...
        for idx, row in enumerate(data_rows):
            ...
            try:
                ...
                make_name = (
                    self._standardize_aircraft_name(make_name) if make_name else ""
                )
                ...
                if not full_model_name or full_model_name in processed_models:
                    _logger.info(
                        "Skipping model - full_model_name: '%s', make_name: '%s', model_name: '%s', variant: '%s'",
                        full_model_name,
                        make_name,
                        model_name,
                        variant,
                ...
                transformed_row.append(
                    make_name
                )  # make_id (name lookup) - can be empty
                ...
                if (
                    engines_idx is not None
                    and engines_idx < len(row)
                    and row[engines_idx]
                ):
                ...
                if sea_idx is not None and sea_idx < len(row) and row[sea_idx]:
                    is_seaplane = str(row[sea_idx]).strip().upper() in (
                        "TRUE",
                        "YES",
                        "1",
                    )
                ...
                if is_multi_engine and is_seaplane:
                    ...
                    not is_multi_engine
                ):  # Default to SEL if we know it's not multi-engine
                    class_id = class_display_names[
                        "class_airplane_sel"
                    ]  # Single-Engine Land
                ...
                engine_type = ""
                if (
                    engine_type_idx is not None
                    and engine_type_idx < len(row)
                    and row[engine_type_idx]
                ):
                ...
                if (
                    tailwheel_idx is not None
                    and tailwheel_idx < len(row)
                    and row[tailwheel_idx]
                ):
                    is_tailwheel = str(row[tailwheel_idx]).strip().upper() in (
                        "TRUE",
                        "YES",
                        "1",
                    )
